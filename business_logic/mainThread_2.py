...
from business_logic.SensorReading import VL53

# Global Variables
sensor_enabled = 1 # Set it 0 to disable sensor dependencies
...
class TakePhoto(QObject):
	...
	def endlessLoop(self):
		# Endless Loop with Sensor Enabled
		try:
			self.sensor.readBytes()
			...
			if self.camera_set == True or self.drawerChanged == 1:
				...
				if self.drawerNum == 1:
					self.map1 , self.map2 , self.map1_0014 ,self.map2_0014 = self.setCameraParameter()
				elif self.drawerNum == 2:
					self.map1 , self.map2 , self.map1_0014 ,self.map2_0014 = self.setCameraParameter120()
					...
					self.map1 , self.map2 , self.map1_0014 ,self.map2_0014 = self.setCameraParameter150()
				else:
					self.map1 , self.map2 , self.map1_0014 ,self.map2_0014 = self.setCameraParameter()
